...
# derived from Jan Collijs docker-jenkins
...
RUN set -x \
    ...
    && apt-get update -y \
    && DEBIAN_FRONTEND="noninteractive" apt-get install -y -o Dpkg::Options::="--force-confnew" --no-install-recommends python-dev \    
    && DEBIAN_FRONTEND="noninteractive" apt-get install -y -o Dpkg::Options::="--force-confnew" --no-install-recommends python-setuptools \
    && DEBIAN_FRONTEND="noninteractive" apt-get install -y -o Dpkg::Options::="--force-confnew" --no-install-recommends python-pip \
    && DEBIAN_FRONTEND="noninteractive" apt-get install -y -o Dpkg::Options::="--force-confnew" --no-install-recommends lsb-release \
